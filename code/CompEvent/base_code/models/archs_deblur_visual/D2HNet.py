...
#                DeblurNet
# ----------------------------------------
class D2HNet(BaseModel):

    def __init__(self,in_nc,out_nc):
        super(D2HNet, self).__init__()


        self.in_channel = in_nc
        self.out_channel = out_nc
        self.activ = 'lrelu'
        self.norm = 'none'
        self.pad_type = 'zero'
        self.deblur_res_num = 8
        self.deblur_res_num2 = 4
        self.final_activ = 'none'
        self.ngf = 64
    ...
    def forward(self, short_img):

        sl = torch.cat([short_img, short_img], dim = 1)
        sl = self.dwt(sl)
        ...
        deblur_out = short_img + deblur_sl
        
        deblur_out = self.final_activ(deblur_out)

        return deblur_out

# my_model = D2HNet(in_nc=3,out_nc=3)
