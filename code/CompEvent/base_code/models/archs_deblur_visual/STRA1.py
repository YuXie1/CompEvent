...
class TMB(nn.Module):
    ...
    def forward(self, event):
        event = F.interpolate(event,scale_factor=0.5)
...
#####################################################################################
#####################################################################################
class STRA1(nn.Module):
    def __init__(self, num_res,base_channel=4):
        super(STRA1, self).__init__()
        self.thita = 1e-3
        # base_channel = 32
    ...
    def forward(self, x,output_last_feature=None):
        ...
        event = x[:,range(3,5),:,:]
        event = torch.cat((event,event,event),1)
