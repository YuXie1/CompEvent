...
class Unet6(nn.Module):
    ...
    def __init__(self,scale):
        ...
        self.scale = scale
        self.fc = nn.Conv2d(3, 32, 5, padding=2)
        # layer 1
        ...
        self.first_conv = nn.Conv2d(45, 3, kernel_size=1, padding=0)

        self.ps1=nn.PixelShuffle(2)
        self.ps2=nn.PixelShuffle(2)
        
        self.last_conv2 = nn.Conv2d(3, 12, kernel_size=1, padding=0)
        self.last_conv = nn.Conv2d(3, 12, kernel_size=1, padding=0)
        self.final_conv = nn.Conv2d(3, 3, kernel_size=1, padding=0)
    def forward(self, x, aggregate_events):
        prev_states = [None] * 4
        x = self.first_conv(x)
        x_org = x
