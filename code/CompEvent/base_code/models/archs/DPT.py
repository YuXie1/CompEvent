...
class CrossAttentionSALSA(nn.Module):
    def __init__(self, ksize=7, stride_1=4, stride_2=4, softmax_scale=10, shape=64, p_len=64, in_channels=64
        ...
        self.W = nn.Conv2d(in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1, stride=1,
                           padding=0)
        self.theta = nn.Conv2d(in_channels=self.in_channels*3, out_channels=self.inter_channels, kernel_size=1, stride=1,
                               padding=0)
        self.phi = nn.Conv2d(in_channels=self.in_channels*3, out_channels=self.inter_channels, kernel_size=1, stride=1,
                             padding=0)
...
class DPT_Net(nn.Module):
    def __init__(self, angRes, factor):
        super(DPT_Net, self).__init__()
        n_blocks, channel = 1, 18
        self.angRes = angRes
        self.factor = factor
        ...
        self.last_conv = nn.Conv2d(25, 256, kernel_size=3, 
                     stride=1, padding=1, bias=True)  
        self.final_conv = nn.Conv2d(256, 3, kernel_size=3, 
                     stride=1, padding=1, bias=True)  
        self.single_conv = nn.Conv2d(45, 3, kernel_size=3, 
                     stride=1, padding=1, bias=True)  
    def forward(self, x,aggregated_event):

        x_upscale = F.interpolate(x, scale_factor=self.factor, mode='bicubic',
                                  align_corners=False)
        # _g = self.get_gradient(x)
        s = self.srbranch(x)
        _d = self.gbranch(x)
        # fuse_feature = self.fuse(s,d)
        fuse_feature = s
        fuse_feature = self.Reconstruct(fuse_feature)
        print('fuse_fature=',fuse_feature.shape)
        out = self.UpSample(fuse_feature)
        print('out=',out.shape)
        # out = out.squeeze()
        
        print('x_upscale=',x_upscale.shape)
        # print('out=',out.shape)
        last = FormOutput(out)
        print('last=',last.shape)
        # out = fuse_feature + x_upscale
        
        out = self.single_conv(x_upscale)
...
class Upsample(nn.Module):
    def __init__(self, blocks,channel, factor):
        ...
        self.upsp = nn.Sequential(
            nn.Conv2d(18, channel * factor * factor, kernel_size=1, stride=1, padding=0, bias=False),
...
class IMDB(nn.Module):
    def __init__(self, channel):
        ...
        self.conv_0 = nn.Conv2d(channel//3, channel, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv_1 = nn.Conv2d(3 * channel // 4 + 1, channel, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv_2 = nn.Conv2d(3 * channel // 4 + 1, channel, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv_3 = nn.Conv2d(3 * channel // 4 + 1, channel // 4, kernel_size=3, stride=1, padding=1, bias=False)
        self.lrelu = nn.LeakyReLU(0.1, inplace=True)
        self.conv_t = nn.Conv2d(52, channel//3, kernel_size=1, stride=1, padding=0, bias=False)
