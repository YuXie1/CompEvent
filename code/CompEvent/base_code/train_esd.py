...
def main():
    ...
    parser.add_argument('-opt', type=str,default='/code/video_deblur/base_code/options/train_ESD/train_ESD.yml', help='Path to option YAML file.')
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        ...
        if opt['use_tb_logger'] and 'debug' not in opt['name']:
            ...
            tb_logger = SummaryWriter(log_dir='/logs/' + opt['name'])
            print('/logs/' + opt['name'])
    else:
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                if opt['model'] in ['sr', 'srgan', 'sd'] and rank <= 0:  # image restoration validation
                    ...
                    for val_data in val_loader:
                        ...
                        rb_img = util.tensor2img(visuals['reblur'])  # uint8
                        gt_img = util.tensor2img(visuals['GT'])  # uint8

                        # Save SR images for reference
                        # save_img_path = os.path.join(img_dir,
                        #                              '{:s}_{:d}.png'.format(img_name, current_step))
                        save_img_path_rb = os.path.join(img_dir,
                                                     '{:s}_{:d}_reblur.png'.format(img_name, current_step))
                        
                        if opt['save_img']:
                            # util.save_img(sr_img, save_img_path)
                            util.save_img(rb_img, save_img_path_rb)
