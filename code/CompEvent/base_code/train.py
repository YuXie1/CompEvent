...
def main():
    ...
    parser.add_argument('-opt', type=str,default='options/train_EFSAI/train_EFSAI_alldata128.yml', help='Path to option YAML file.')
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        ...
        if opt['use_tb_logger'] and 'debug' not in opt['name']:
            ...
            tb_logger = SummaryWriter(log_dir='/output/' + opt['name'])
    else:
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            if opt['dist']:
                ...
                train_sampler = None
            train_loader = clreate_dataloader(train_set, dataset_opt, opt, train_sampler)
            if rank <= 0:
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                if opt['model'] in ['sr', 'srgan', 'sai'] and rank <= 0:  # image restoration validation
